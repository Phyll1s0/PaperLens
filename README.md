...
长任务提速可以在 `.env` 调整批次大小：`PAPERLENS_AGENT_ANALYSIS_BATCH_SIZE=6` 控制 Claude Code Agent，`PAPERLENS_ANALYSIS_BATCH_SIZE=4` 控制普通 OpenAI-compatible。批次越大越快，但单次输出越长，模型越容易返回不完整 JSON；建议不要超过 8。
