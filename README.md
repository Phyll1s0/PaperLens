...
长任务默认按“约 20 分钟出一篇”优化：后端会把段落合并成快速批次，压缩上下文，并把讲解限制为一句话，还会并发跑多个批次并安全合并写入。`.env` 可调整：`PAPERLENS_TARGET_ANALYSIS_MINUTES=20` 控制目标耗时，`PAPERLENS_AGENT_ANALYSIS_BATCH_SIZE=14` / `PAPERLENS_AGENT_ANALYSIS_CONCURRENCY=2` 控制 Claude Code Agent，`PAPERLENS_ANALYSIS_BATCH_SIZE=18` / `PAPERLENS_ANALYSIS_CONCURRENCY=3` 控制普通 OpenAI-compatible。批次和并发越大越快，但更容易触发限流、超时或 JSON 不完整；如果频繁拆分重试，把批次降到 8-12，或把并发降到 1。
