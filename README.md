...
如果你的网络必须通过代理访问模型服务，推荐在 `.env` 里设置 `PAPERLENS_PROXY_URL`。本机后台脚本、macOS launchd 和 Docker Compose 都会读取它；网页模型诊断里也会显示 `Proxy: detected`。也可以直接设置 `HTTP_PROXY` / `HTTPS_PROXY` / `ALL_PROXY`。
