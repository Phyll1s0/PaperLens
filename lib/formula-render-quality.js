...
export function shouldExportFormulaTextAsAuxiliary(artifact = {}) {
  ...
  return fields.renderMode === FORMULA_RENDER_MODE_IMAGE_LATEX &&
    (fields.latexConfidence === "low" || fields.latexConfidence === "medium") &&
    Boolean(normalizeFormulaText(artifact.latex || artifact.text || ""));
}

export function getFormulaAuxiliaryTextLabel(artifact = {}) {
  const fields = buildFormulaRenderFields(artifact);
  if (fields.latexConfidence === "low") {
    return "识别文本（低置信，仅供核对）";
  }
  return "识别文本（图片优先，供核对）";
}
...
function inferFormulaRenderMode({ artifact, text, latexConfidence, source }) {
  ...
  if (latexConfidence === "medium" && hasCrop && isAutoExtractedFormulaTextSource(source)) {
    return FORMULA_RENDER_MODE_IMAGE_LATEX;
  }
...
function isAutoExtractedFormulaTextSource(source) {
  return source === "pdf-text" || source === "ocr-text";
}
