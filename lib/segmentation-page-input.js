...
export function getReadablePageBlocks(page, options = {}) {
  if (Array.isArray(page?.blocks) && page.blocks.length) {
    ...
    for (const rawBlock of page.blocks) {
      const block = {
        ...rawBlock,
        originalIndex: Number.isFinite(Number(rawBlock?.originalIndex)) ? Number(rawBlock.originalIndex) : blocks.length,
    ...
    if (blocks.length) {
      return sortReadableBlocksForSegmentation(blocks, page);
    }
  }
