...
export function isLikelyTableBodyBlockText(text, block = {}) {
  ...
  if (lineCount < 2 || sentenceCount >= 2) {
    return false;
  }

  const numberTokens = countMatches(clean, /\b\d+(?:[.,]\d+)*%?\b/g);
  const citationTokens = countMatches(clean, /\[[0-9,\s]+\]/g);
  const tableWords = countMatches(clean, /\b(?:architecture|scaling|factor|data\s+type|data|type|granularity|format|metadata|content|model|dataset|method|metric|mae|mse|rmse|accuracy|perplexity|latency|throughput|average|baseline|ours|layers?|heads?|vocab|params?|dmodel|dff|fp\d+|int\d+|mxfp|nvfp|smx|olive|tender|mant|bitmod|group-\d+|tensor\/channel)\b/gi);
  const columnish = lineCount >= 4 && averageLineLength <= 38 && (numberTokens + citationTokens + tableWords) >= 3;
  const compactHeader = lineCount <= 4 && averageLineLength <= 76 && clean.length <= 260 && tableWords >= 3 && sentenceCount === 0;
  if (averageLineLength > 48 && !compactHeader) {
    return false;
  }
