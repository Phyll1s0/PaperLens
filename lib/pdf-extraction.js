...
export async function extractPdfText(pdfPath, assetDir = "", assetPublicBase = "", options = {}) {
  ...
  if (requestedEngine === "fixture") {
    return await extractPdfWithFixture(pdfPath, assetDir, assetPublicBase);
  }
...
export async function extractPdfWithFixture(pdfPath, assetDir = "", assetPublicBase = "") {
  if (assetDir && assetPublicBase) {
    await mkdir(assetDir, { recursive: true });
  }

  const imagePath = assetDir && assetPublicBase ? `${assetPublicBase}/page-001.png` : null;
  if (assetDir && imagePath) {
    await writeFile(path.join(assetDir, "page-001.png"), makeFixturePng(300, 220, [
      { x: 64, y: 102, width: 72, height: 34 },
      { x: 172, y: 102, width: 72, height: 34 },
      { x: 72, y: 188, width: 150, height: 18 },
    ]));
  }
  ...
  const pages = [{
    ...
    text: [
      "PaperLens Fixture",
      "This upload fixture paragraph verifies extraction segmentation and queued analysis.",
      "A second readable paragraph keeps the upload pipeline closer to a real paper.",
      "(a) Input Query Chunk (b) Output Summary Checker",
      "Figure 1. PaperLens visual chain fixture.",
      "y = W x + b + θ (1)",
    ].join("\n\n"),
    blocks: [
      {
        text: "PaperLens Fixture",
        x: 20,
        y: 18,
        width: 180,
        height: 20,
        lineCount: 1,
        lines: [{ text: "PaperLens Fixture", x: 20, y: 18, width: 180, height: 20 }],
      },
      {
        text: "This upload fixture paragraph verifies extraction segmentation and queued analysis.",
        x: 20,
        y: 46,
        width: 250,
        height: 20,
        lineCount: 1,
        lines: [{ text: "This upload fixture paragraph verifies extraction segmentation and queued analysis.", x: 20, y: 46, width: 250, height: 20 }],
      },
      {
        text: "A second readable paragraph keeps the upload pipeline closer to a real paper.",
        x: 58,
        y: 70,
        width: 220,
        height: 20,
        lineCount: 1,
        lines: [{ text: "A second readable paragraph keeps the upload pipeline closer to a real paper.", x: 58, y: 70, width: 220, height: 20 }],
      },
      {
        text: "(a) Input Query Chunk\n(b) Output Summary Checker",
        x: 58,
        y: 102,
        width: 190,
        height: 34,
        lineCount: 2,
        lines: [
          { text: "(a) Input Query Chunk", x: 58, y: 102, width: 180, height: 16 },
          { text: "(b) Output Summary Checker", x: 58, y: 120, width: 190, height: 16 },
        ],
      },
      {
        text: "Figure 1. PaperLens visual chain fixture.",
        x: 56,
        y: 148,
        width: 210,
        height: 16,
        lineCount: 1,
        lines: [{ text: "Figure 1. PaperLens visual chain fixture.", x: 56, y: 148, width: 210, height: 16 }],
      },
      {
        text: "y = W x + b + θ (1)",
        x: 70,
        ...
        lineCount: 2,
        lines: [
          { text: "y = W x + b + θ", x: 70, y: 186, width: 130, height: 16 },
          { text: "(1)", x: 204, y: 202, width: 18, height: 10 },
        ],
      },
    ],
    width: 300,
    height: 220,
    imagePath,
    imageWidth: imagePath ? 300 : null,
    imageHeight: imagePath ? 220 : null,
  }];
