...
import {
  isProtectedSemanticLeadInText,
} from "./segmentation-page-input.js";

const MAX_DEBUG_PAGES = 80;
const MAX_BLOCKS_PER_PAGE = 80;
const MAX_PARAGRAPHS = 180;
const MAX_EXTRA_RECOVERABLE_PARAGRAPHS = 80;
...
export function buildPaperSegmentationDebugReport(paper, options = {}) {
  ...
  const debugParagraphs = selectDebugParagraphs(paragraphs, Number(options.maxParagraphs || MAX_PARAGRAPHS));
  const paragraphReports = debugParagraphs
  ...
  const recoverableFilteredParagraphs = paragraphReports.filter((paragraph) => paragraph.recoverableFilteredBlock);
  ...
  return {
    ...
    summary: {
      ...
      recoverableFilteredParagraphs: recoverableFilteredParagraphs.length,
...
function selectDebugParagraphs(paragraphs, limit) {
  const baseLimit = Math.max(1, Number(limit || MAX_PARAGRAPHS));
  const selected = paragraphs.slice(0, baseLimit);
  const selectedIds = new Set(selected.map((paragraph) => paragraph?.id).filter(Boolean));
  let extraRecoverable = 0;

  for (const paragraph of paragraphs) {
    if (extraRecoverable >= MAX_EXTRA_RECOVERABLE_PARAGRAPHS) {
      break;
    }
    if (!paragraph?.recoverableFilteredBlock || selectedIds.has(paragraph.id)) {
      continue;
    }
...
function buildSegmentationIssueSummary(pageReports, paragraphReports) {
  ...
  for (const paragraph of paragraphReports || []) {
    if (paragraph.recoverableFilteredBlock) {
      addSegmentationIssueSample(categories, "recoverable-filtered", {
        source: "paragraph",
        ...
        reasons: [
          "recoverable-filtered-block",
          paragraph.recoverableFilteredBlock.reason || "",
        ].filter(Boolean),
      });
    }

    for (const reason of paragraph.noiseReasons || []) {
      if (paragraph.recoverableFilteredBlock && reason === "recoverable-filtered-block") {
        continue;
      }
...
function createSegmentationIssueCategories() {
  return {
    ...
    resource: createIssueCategory("resource", "资源链接", "low", "代码、数据集、模型或 benchmark 链接已保留为隐藏资源线索，可按需检查。"),
    ...
    "recoverable-filtered": createIssueCategory("recoverable-filtered", "已过滤但可恢复", "low", "这些内容默认不进入讲解队列，但已作为隐藏段落保留，可打开隐藏段落检查。"),
...
function getIssueCategoryForBlockReason(reason) {
  const map = {
    ...
    "recoverable-filtered-block": "recoverable-filtered",
  };
  return map[reason] || "";
}
...
function buildParagraphDebug(paper, paragraph, index, flatBlocks) {
  ...
  return {
    ...
    mergeTrace: Array.isArray(paragraph?.segmentationMergeTrace) ? paragraph.segmentationMergeTrace.slice(-6) : [],
    noiseReasons,
    segmentationNoise: paragraph?.segmentationNoise || null,
    recoverableFilteredBlock: paragraph?.recoverableFilteredBlock || null,
  };
}
...
function getDropReasons(rawText, cleanText, context) {
  ...
  const protectedLeadIn = isProtectedSemanticLeadInText(cleanText);
  if (protectedLeadIn) {
  ...
  if (isLikelyStandaloneLinkText(cleanText)) {
    reasons.push(isLikelyResourceLinkText(cleanText) ? "resource-link" : "standalone-link");
  }
...
function getBlockTags(rawText, cleanText, context, reasons) {
  ...
  if (isProtectedSemanticLeadInText(cleanText)) {
    tags.push("semantic-lead-in");
  }
...
function getParagraphNoiseReasons(paragraph, section) {
  ...
  if (Array.isArray(paragraph?.segmentationNoise?.reasons)) {
    reasons.push(...paragraph.segmentationNoise.reasons);
  }
  if (paragraph?.recoverableFilteredBlock) {
    reasons.push("recoverable-filtered-block");
    if (paragraph.recoverableFilteredBlock.reason) {
      reasons.push(paragraph.recoverableFilteredBlock.reason);
    }
  }
  return uniqueStrings(reasons);
}
...
function buildReasonLegend() {
  return {
    ...
    "resource-link": "代码、数据集、模型或 benchmark 等资源链接。",
    "semantic-lead-in": "定义、定理、备注、takeaway 等正文结构引导语。",
    ...
    "recoverable-filtered-block": "内容已从正文输入过滤，但作为隐藏段落保留。",
    hidden: "当前段落被用户或系统隐藏。",
  };
}
