...
export function buildPaperSegmentationDebugReport(paper, options = {}) {
  ...
  const pageReports = pages.slice(0, MAX_DEBUG_PAGES).map((page) => buildPageDebug(page));
...
function isLikelyTableBodyText(text, context = {}) {
  ...
  const tableTokens = /\b(dataset|granularity|method|model|metric|mae|mse|rmse|accuracy|precision|recall|baseline|ours|avg|mean)\b/i.test(text);
