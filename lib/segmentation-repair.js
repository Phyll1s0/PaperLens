...
const INLINE_ARXIV_STAMP_RE = new RegExp(
  `\\s*arXiv:\\d{4}\\.\\d{4,5}(?:v\\d+)?(?:\\s*\\[[^\\]]{1,80}\\])?(?:\\s+\\d{1,2}\\s+(?:${MONTH_PATTERN})\\s+\\d{4})?\\s*`,
  "gi",
);
...
function replaceInlineMetadataFragment(source, pattern) {
  pattern.lastIndex = 0;
  const next = String(source || "").replace(pattern, (match, offset, whole) => {
...
export function isLikelyPageNumberOrRunningHeaderText(text) {
  ...
  if (/^[A-Z][A-Za-z'’-]+(?:\s+[A-Z][A-Za-z'’-]+){0,3}\s+et\s+al\.?$/i.test(clean)) {
    return true;
  }

  if (/^(?:\d+(?:\.\d+)?\s+)*[A-Z][A-Za-z'’-]+(?:\s+[A-Z][A-Za-z'’-]+){0,3}\s+et\s+al\.?(?:\s+\d+(?:\.\d+)?)*$/i.test(clean)) {
    return true;
  }

  if (isLikelyPaperTitleRunningHeaderText(clean)) {
    return true;
  }
...
function isLikelyPaperTitleRunningHeaderText(text) {
  const clean = normalizeSegmentationText(text);
  if (clean.length < 38 || clean.length > 120 || /[.!?。！？]$/.test(clean)) {
    return false;
  }
  if (/^\d+(?:\.\d+)*\s+/.test(clean)) {
    return false;
  ...
  const titleishWords = (clean.match(/\b[A-Z][A-Za-z0-9+-]{2,}\b/g) || []).length;
  const acronymWords = (clean.match(/\b[A-Z0-9]{2,}\b/g) || []).length;
  const hasTitleSeparator = /[:：]|[–—-]/.test(clean);
  return titleishWords + acronymWords >= 5 && hasTitleSeparator;
}

export function isLikelyFrontMatterTitleText(text, context = {}) {
  const clean = normalizeSegmentationText(text);
  const pageNumber = Number(context?.pageNumber || 0);
  const y = Number(context?.y ?? context?.top ?? 0);
  const lineCount = Number(context?.lineCount || 1);
  if (pageNumber > 1 || y > 180 || lineCount > 4 || clean.length < 20 || clean.length > 220) {
    return false;
  }

  if (isLikelyPublicationMetadataText(clean) || isReferencesSectionTitleText(clean)) {
    return false;
  }

  if (/^(?:abstract|introduction|keywords?|acknowledg(?:e)?ments?)$/i.test(clean)) {
    return false;
  }

  if (/[.!?。！？]$/.test(clean) || /@|https?:\/\/|www\.|doi\.org/i.test(clean)) {
    return false;
  }

  const letterCount = (clean.match(/\p{L}/gu) || []).length;
  return letterCount >= 12 && /[A-Z]/.test(clean);
}
...
export function stripPublicationMetadataFragments(text) {
  ...
  let next = replaceInlineMetadataFragment(source, INLINE_ARXIV_STAMP_RE);
  next = replaceInlineMetadataFragment(next, CONFERENCE_DATE_LOCATION_RE);
