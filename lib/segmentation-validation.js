...
export function scoreCrossPageMergeCandidate(previous, paragraph, options = {}) {
  ...
  const hyphenContinuation = String(previous.sourceText || "").endsWith("-");
  ...
  if (shouldMergeSegmentedText(previous.sourceText || "", paragraph.sourceText || "", {
    sameSection: !softBlockers.length || hasSameSectionSignal(previous, paragraph),
  ...
  const preliminaryShouldMerge = explicitContinuation ||
    decision.score >= 6 && textualSignal ||
    decision.score >= 7 && boundarySignal && previousOpen;
  const canOverrideSectionMismatch = explicitContinuation || hyphenContinuation;
  decision.shouldMerge = preliminaryShouldMerge && (!softBlockers.length || canOverrideSectionMismatch);
  if (preliminaryShouldMerge && softBlockers.length && canOverrideSectionMismatch) {
    addCrossPageScore(decision, 1, "section-mismatch-overridden");
  }
  if (!decision.shouldMerge) {
    decision.blockers.push(...softBlockers);
...
export function auditSegmentedParagraphNoise(paragraph, structureMap = null, repeatedTextIndex = new Map()) {
  ...
  if (isLikelyKnownPaperTitleHeaderText(clean, structureMap, context)) {
    reasons.push("paper-title-header");
  }
...
function isLikelyKnownPaperTitleHeaderText(text, structureMap = null, context = {}) {
  const clean = normalizeSegmentationParagraph(text);
  const title = normalizeSegmentationParagraph(structureMap?.paperTitle || structureMap?.title || "");
  const pageNumber = normalizePositivePageNumber(context?.pageNumber, 0);
  if (!clean || !title || pageNumber <= 1 || clean.length < 12 || clean.length > 180) {
    return false;
  }

  if (/^(?:abstract|introduction|background|method|approach|evaluation|experiments?|results?|discussion|conclusion|appendix|references)\b/i.test(clean)) {
    return false;
  }

  if (/[.!?。！？]$/.test(clean) && !/[?:：]$/.test(title)) {
    return false;
  }

  const textKey = normalizeTitleHeaderKey(clean);
  const titleKey = normalizeTitleHeaderKey(title);
  if (!textKey || !titleKey) {
    return false;
  }
  ...
  const textTokens = getTitleHeaderTokens(textKey);
  const titleTokens = getTitleHeaderTokens(titleKey);
  if (textTokens.length < 3 || titleTokens.length < 4) {
    return false;
  }

  if ((titleKey.includes(textKey) || textKey.includes(titleKey)) && textTokens.length >= 4) {
    return true;
  }

  const titleTokenSet = new Set(titleTokens);
  const overlap = textTokens.filter((token) => titleTokenSet.has(token)).length;
  return overlap >= 4 && overlap / Math.max(1, textTokens.length) >= 0.78;
}

function normalizeTitleHeaderKey(text) {
  return String(text || "")
    .normalize("NFKD")
    .toLowerCase()
    .replace(/[^\p{L}\p{N}]+/gu, " ")
...
function getTitleHeaderTokens(text) {
  ...
  return normalizeTitleHeaderKey(text)
    .split(/\s+/)
