...
- [ ] 测试覆盖不足：核心路径仍以 `node --check`、health 和手工 API 测试为主，缺少 Provider 诊断回归测试。
- [ ] 长任务预算还不够透明：分析前缺少每篇论文预计 token、预计时间、预计费用/额度和任务级预算上限。
- [ ] 视觉结构仍不够可控：复杂双栏、多图组合、跨页图、公式/代码混排时，自动裁剪仍可能过大、误分类，用户还不能手动修正。
- [ ] 精读分段本地兜底仍需继续增强：复杂双栏论文里，References、表格主体、流程图文字、跨页自然段仍可能被误收或误合并。
- [ ] 跨页定位体验仍需升级：段落跨页、图表跨页和相关图表跳转需要更明确的页码锚点与整页定位反馈。

## Next

1. [ ] 精读分段回归集：用 M2XFP/Chronos/Kronos 这几篇本地样例固化 References、表格块、页眉页脚、跨页段落的回归测试。
2. [ ] 自动化测试扩展：补 Provider 诊断回归测试。
