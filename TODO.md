...
- [ ] 测试覆盖仍需扩展：已有 Provider/Kimi/导出/Job/PDF/富文本/分段噪声/分段验证/模型诊断/视觉重建摘要样例测试，但缺少真实 PDF 视觉 fixture 回归测试。
...
1. [ ] 真实 PDF 视觉 fixture 回归：补真实页面图裁剪链路测试，覆盖页面图生成、视觉块识别、裁剪 SVG 输出和低置信摘要。
